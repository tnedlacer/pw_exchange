class PwRequestsController < ApplicationController
  ...
  def create
    if [params[:public_key], KeyManager.instance.public_key].map{|str| str.to_s.gsub(/\s/, "") }.uniq.size != 1 || params[:pw_request].blank?
    ...
    pw_request.assign_encrypted_attributes(params.require(:pw_request).permit(:password, :email))
    ...
    if pw_request.save
      ...
      render js: "$(\"#pw_submit\").hide(); $(\"div.input_section\").html(\"#{html[:input_field]}\"); $(\"div#content\").append(\"#{html[:step2]}\").append(\"#{html[:step3]}\");;"
    else
      ...
      render js: "$(\"div.input_section\").html(\"#{input_field_html}\");$.rails.enableElement($(\"#pw_submit\"));"
    end
  end
end
